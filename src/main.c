...
int work_with_params (long long* restrict from , long long* restrict to , int number_of_params, char* params[] )
{
    ...
    if(((second_flag_for_from == 1 ) && (second_flag_for_to) ==1 ) && ((*(from) == 0) && (*(to) == 0) )) // Если 2 раза было введено некоректное число, то это ошибка (-4)
    {
        return -4;
    }
    else if(flag_for_from == 1 && ((*(to) != 0) || (second_flag_for_to == 2)))       // Если пользователь намеренно не ввёл переменную, то нужно присвоить ей минимально возможное значение
    {
        *(from) = LLONG_MIN;
    }
    else if(flag_for_to == 1 && ((*(from) != 0) || (second_flag_for_from == 2)))     // Если пользователь намеренно не ввёл переменую, то нужно ей присвоить максимально возможное значение
    {
        *(to) = LLONG_MAX;
    }
